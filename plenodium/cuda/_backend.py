...
try:
    ...
    from plenodium import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "water_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "water_cuda.lib")
        ):
            ...
            with Console().status(
                "[bold yellow]water: Setting up CUDA (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]water: No CUDA toolkit found. water will be disabled.[/yellow]"
        )
