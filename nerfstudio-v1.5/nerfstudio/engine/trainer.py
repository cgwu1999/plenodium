...
from nerfstudio.utils.writer import EventName, TimeWriter, _format_time
...
class TrainerConfig(ExperimentConfig):
    ...
    direct_close: bool = False
    """Whether to close the viewer directly after training."""
    results_output: Optional[Path] = None
    """Path to save results to."""
    save_images: bool = False
    """Whether to save images during training."""
...
class Trainer:
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                self.train_time += train_t.duration
                ...
                if self.pipeline.datamanager.eval_dataset:
                    with self.train_lock:
                        metrics_dict=self.eval_iteration(step)
                        if metrics_dict is None and step_check(step, self.config.max_num_iterations - 1):
                            metrics_dict = self.final_eval_iteration(step)
                            table = Table(
                                title=None,
                                show_header=False,
                                box=box.MINIMAL,
                            ...
                            table.add_row("PSNR", "SSIM", "LPIPS", "fps")
                            table.add_row(str(metrics_dict["psnr"]), str(metrics_dict["ssim"]), str(metrics_dict['lpips']), str(metrics_dict['fps']))
                            CONSOLE.print(Panel(table, title="[bold][green]:tada: EVALUATION RESULTS:tada:[/bold]", expand=False))
        ...
        if self.config.results_output:
            with open(self.config.results_output, "a") as f:
                f.write(f'{metrics_dict["psnr"]}\t{metrics_dict["ssim"]}\t{metrics_dict["lpips"]}\t{metrics_dict["fps"]}\t{self.train_time}\n')
    ...
    def _after_train(self) -> None:
        ...
        table.add_row("Total Training Time", _format_time(self.train_time))
    ...
    def _train_complete_viewer(self) -> None:
        ...
        try:
            ...
            time.sleep(0.03)  # sleep to allow buffer to reset
            CONSOLE.log("Viewer failed. Continuing training.")
        if self.config.direct_close:
            CONSOLE.print("The Task Done!", justify="center")
        else:
            CONSOLE.print("Input Q to quit", justify="center")
            # while True:
            # time.sleep(0.01)
    ...
    def eval_iteration(self, step: int) -> None:
        ...
        if step_check(step, self.config.steps_per_eval_all_images):
            output_path = (
                Path(f"{self.config.get_output_dir()}/{self.step}")
    ...
    def final_eval_iteration(self, step: int) -> Dict:
        output_path = (
            Path(f"{self.config.get_output_dir()}/{self.step}")
            if self.config.save_images
            else None
        )
        return self.pipeline.get_average_eval_image_metrics(
            step=step, output_path=output_path
        )
        # writer.put_dict(name="Eval Images Metrics Dict (all images)", scalar_dict=metrics_dict, step=step)
